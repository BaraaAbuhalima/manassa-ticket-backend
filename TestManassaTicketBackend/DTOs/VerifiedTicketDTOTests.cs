...
public class VerifiedTicketDTOTests
{
    ...
    [Test]
    public void Constructor_CalculatesTotalPriceJod_FromRawAmount_NotByConvertingTotalPriceBack()
    {
        var transfer = CreateTransferTicket(ticketAmount: "20", luggageCount: 3);

        var result = new VerifiedTicketDTO(transfer);

        // 20 JOD + 3 bags * 2 JOD (a flat JOD fee, independent of the USD bag fee used for
        // TotalPrice) — computed directly from the raw JOD amount, not derived from USD.
        result.TotalPriceJod.Should().Be(26m);
    }
