...
public class TicketDeleterTests
{
    ...
    {
        // originalPrice (JOD) = 9 -> cap = 10 JOD. A price of exactly 10 lands right on the cap.
        var ticket = CreateTicket(pin: "PRICE-1", status: TicketSellStatus.ForSale, originalPrice: 9m);
        _dbContext.Tickets.Add(ticket);
        await _dbContext.SaveChangesAsync();
        var token = _deleteTokenService.GenerateToken(ticket.Id);
    ...
    {
        // originalPrice (JOD) = 9 -> cap = 10 JOD. A price of 10.1 is just over the cap.
        var ticket = CreateTicket(pin: "PRICE-2", status: TicketSellStatus.ForSale, originalPrice: 9m);
        _dbContext.Tickets.Add(ticket);
        await _dbContext.SaveChangesAsync();
        var token = _deleteTokenService.GenerateToken(ticket.Id);
