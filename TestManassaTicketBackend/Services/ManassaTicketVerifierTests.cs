...
public class ManassaTicketVerifierTests
{
    ...
    private static ManassaTicketVerifier CreateSut(Func<HttpRequestMessage, HttpResponseMessage> respond, out FakeHttpMessageHandler handlerRef)
    {
        var options = Options.Create(new ManassaApiOptions { BaseUrl = "https://jett-khb.com.jo/api/" });
        var handler = new FakeHttpMessageHandler(respond);
        handlerRef = handler;
        var client = new HttpClient(handler);
        return new ManassaTicketVerifier(client, options, NullLogger<ManassaTicketVerifier>.Instance);
    }
